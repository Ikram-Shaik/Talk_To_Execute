...
def get_greeting():
    ...
    if 5 <= hour < 12:
        return "Good morning partner."
    elif 12 <= hour < 17:
        return "Good afternoon partner."
    else:
        return "Good evening partner."

# Generate greeting and speak
greeting = get_greeting()
speak(f"{greeting} Whats on your mind?")
...
def main(query):
        ...
        if ("google" in query and "search" in query) or ("google" in query and "how to" in query) or "google" in query or "search" in query:
            googleSearch(query)
            return
        ...
        elif intent == "email" and ("email" in query or "mail" in query):
        ...
        elif intent=='translate' and 'translate' in query:
            speak('Supported Languages are Telugu, Tamil and Hindi')
            speak('In which language you would like translate')
            lang=record()
            speak('Speak to tranlate')
            text=record()
            speak_translated_text(text,lang)
            done =True
        elif intent == "shut_down":
            speak("Shutting the system down")
            speak("Have a nice day")
            os.system("shutdown /s /t 0")
        elif intent == "restart":
            speak("Restarting the system down")
            speak("Wait a second")
            os.system("shutdown /r /t 0")
        elif intent== "brightness" and "brightness" in query:
            brightness=system_brightness(query)
            speak(brightness)
            done = True
        elif intent == "volume" and "volume" in query:
            volume=system_volume(query)
            speak(volume)
            done = True
